    ...
    public static function getCases($allCases = TRUE, $params = [], $context = 'dashboard', $getCount = FALSE)
    {
        ...
        foreach ($result->fetchAll() as $case) {
            ...
            $casesList[$key]['activity_list'] = sprintf(
                '<a title="%s" class="crm-expand-row" href="%s"></a>',
                ts('Activities'),
            ...
            $casesList[$key]['sort_name'] = sprintf(
                '<a href="%s">%s</a>%s<br /><span class="description">%s: %d</span>',
            ...
            if ($actId) {
                if (self::checkPermission($actId, 'view', $case['activity_type_id'], $userID)) {
                    if ($type == 'recent') {
                        $casesList[$key]['date'] = sprintf(
                            '<a class="action-item crm-hover-button" href="%s" title="%s">%s</a>',
                        ...
                        $casesList[$key]['date'] = sprintf(
                            '<a class="crm-popup %s" href="%s" title="%s">%s</a> &nbsp;&nbsp;',
                            $status,
                            CRM_Utils_System::url('civicrm/case/activity/view', ['reset' => 1, 'cid' => $case['contact_id'], 'aid' => $case['activity_id']]),
                            ts('View activity'),
                            $activityTypeLabels[$case['activity_type_id']] ?? ''
                ...
                if (isset($case['activity_type_id']) && self::checkPermission($actId, 'edit', $case['activity_type_id'], $userID)) {
                    $casesList[$key]['date'] .= sprintf(
                        '<a class="action-item crm-hover-button" href="%s" title="%s"><i class="crm-i fa-pencil" aria-hidden="true"></i></a>',
            ...
            $casesList[$key]['links'] = CRM_Core_Action::formLink(
        ...
        if (!empty($managerRoleId)) {
          ...
          SELECT civicrm_contact.id as casemanager_id,
           ...
           WHERE civicrm_case.id = %2";
            ...
            if (!empty($activekey)) {
                $caseManagerName = sprintf(
                    '<a href="%s">%s</a>',
                    CRM_Utils_System::url('civicrm/contact/view', ['cid' => $activekey]),
                    $caseManagerNameArray[$activekey]['casemanager']
                );
                ...
                $caseManagerName = sprintf(
                    '<a href="%s">%s</a>',
                    CRM_Utils_System::url('civicrm/contact/view', ['cid' => $max]),
                    $caseManagerNameArray[$max]['casemanager']
                );
            }
        }
    ...
    public static function buildPermissionLinks(&$tplParams, $activityParams)
    {
        ...
        if (!empty($tplParams['isCaseActivity'])) {
            $tplParams['editActURL'] = CRM_Utils_System::url(
                ...
                "reset=1&cid={$activityParams['target_id']}&caseid={$activityParams['case_id']}&action=update&id={$activityParams['source_record_id']}",
                TRUE
            );

            $tplParams['viewActURL'] = CRM_Utils_System::url(
                'civicrm/case/activity/view',
                "reset=1&aid={$activityParams['source_record_id']}&cid={$activityParams['target_id']}&caseID={$activityParams['case_id']}",
                TRUE
            );

            $tplParams['manageCaseURL'] = CRM_Utils_System::url(
                'civicrm/contact/view/case',
                "reset=1&id={$activityParams['case_id']}&cid={$activityParams['target_id']}&action=view&context=home",
                TRUE
            );
        } else {
            $tplParams['editActURL'] = CRM_Utils_System::url(
                'civicrm/contact/view/activity',
                "atype=$activityTypeId&action=update&reset=1&id={$activityParams['source_record_id']}&cid={$tplParams['contact']['contact_id']}&context=activity",
                TRUE
            );

            $tplParams['viewActURL'] = CRM_Utils_System::url(
                'civicrm/contact/view/activity',
                "atype=$activityTypeId&action=view&reset=1&id={$activityParams['source_record_id']}&cid={$tplParams['contact']['contact_id']}&context=activity",
                TRUE
            );
        }
    }
    ...
    public static function checkPermission($activityId, $operation, $actTypeId = NULL, $contactId = NULL, $checkComponent = TRUE)
    {
        ...
        if ($allow) {
            ...
            if ($allow && in_array($operation, $actionOperations)) {
                ...
                if (
                    array_key_exists($operation, $actionFilter[$operation]) &&
